...
const float MAX_JUMP = 300; // Máxima diferencia permitida entre lecturas consecutivas
...
void loop() {
  ...
  int valor = digitalRead(pinEntrada);

  // Si el pin está en alto
  if (valor == HIGH) {
    Serial.println("El pin está en ALTO, realizando acción A...");
    // Aquí pones el código para lo que debe hacer cuando el pin esté en alto
  }
  // Si el pin está en bajo
  else {
    Serial.println("El pin está en BAJO, realizando acción B...");
    // Aquí pones el código para lo que debe hacer cuando el pin esté en bajo
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    String message = variable_luces; // El mensaje que se enviará
